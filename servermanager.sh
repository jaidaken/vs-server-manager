...
class VintageStoryServerManager:
    def __init__(self):
        ...
        self._dirty_regions = {'header', 'log', 'players', 'feedback', 'help', 'input'}
    ...
    def check_dependencies(self):
        ...
        for cmd, package in required_commands.items():
            try:
                result = subprocess.run(['which', cmd], capture_output=True, timeout=1.0)
            ...
            with self._player_update_lock:
                if self.get_process_info()['is_running']:
                    if self.send_command("list clients"):
                        # Non-blocking: parse immediately without waiting
    ...
    def get_pid(self) -> Optional[int]:
        ...
        if self.pid_update_needed:
            # Non-blocking: update PID immediately without delays
    ...
    def force_log_refresh(self):
        ...
        # Non-blocking: refresh immediately without waiting
        try:
    ...
    def restart_sequence(self):
        """Background thread for restart sequence with non-blocking 20-second wait"""
        # Send stop command
        self.add_command_log("Sending stop command...")
        self.stop_server()

        # Start background timer for 20-second wait
        self.add_command_log("Waiting 20 seconds for server shutdown...")
        self.start_background_task(self._restart_timer)

    def _restart_timer(self):
        """Background timer that waits 20 seconds then starts the server"""
        import time

        # Wait 20 seconds in background
        start_time = time.time()
        while time.time() - start_time < 20:
            time.sleep(0.1)  # Check every 100ms to allow for early termination
            # Check if we should abort (if manager is shutting down)
            if not self.running:
                self.add_command_log("Restart cancelled - manager shutting down")
                return

        # Start new server after 20 seconds
        self.add_command_log("Starting new server...")
    ...
    def log_monitor_thread(self):
                ...
                with open(self.log_file, 'r', encoding='utf-8', errors='ignore') as f:
                    ...
                    if existing_content:
                        ...
                        with self.log_lock:
                            ...
                            self.scroll_offset = 0
                            # Mark log area as dirty after loading existing content
                            self.mark_dirty('log')
                            self._needs_redraw = True
                            # Force immediate redraw by clearing content cache
                            self._last_log_content = ""

            except Exception as e:
                self.log_error(f"Error reading existing log: {e}")

        # File monitoring with reduced I/O frequency
        check_interval = 0.05  # Check every 50ms for more responsive updates
        last_check = 0
        ...
        while self.log_thread_running:
            try:
                            ...
                            with open(self.log_file, 'r', encoding='utf-8', errors='ignore') as f:
                                ...
                                if new_content:
                                    ...
                                    with self.log_lock:
                                        ...
                                        # Auto-scroll to newest lines when new content is added
                                        self.scroll_offset = 0
                                        # Force redraw of log area
                                        self.mark_dirty('log')
                                        self._needs_redraw = True
                                        # Force immediate redraw by clearing content cache
                                        self._last_log_content = ""
                ...
                # Log file I/O error, continue monitoring
                time.sleep(check_interval)
            except Exception as e:
                # Unexpected error in log monitoring
                self.log_error(f"Unexpected error in log monitoring: {e}")
                time.sleep(check_interval)
    ...
    def player_update_thread(self):
        ...
        while self.player_update_running:
            try:
                ...
                self.log_error(f"Player update I/O error: {e}")
                time.sleep(5)  # Reduced sleep on error
            except Exception as e:
                # Unexpected error in player update
                self.log_error(f"Player update unexpected error: {e}")
                time.sleep(5)  # Reduced sleep on error
    ...
    def start_player_update_thread(self):
        ...
        def safe_initial_check():
            try:
                self.initial_player_check()
            except Exception as e:
                self.log_error(f"Initial player check error: {e}")

        threading.Thread(target=safe_initial_check, daemon=True).start()
    ...
    def stop_player_update_thread(self):
        ...
        if self.player_update_thread and self.player_update_thread.is_alive():
            self.player_update_thread.join(timeout=0.5)  # Reduced timeout
    ...
    def draw_log_area(self):
        ...
        log_start_y = 2
        log_end_y = self.max_y - 6  # Account for feedback frame and help lines (no borders)
        ...
        log_width = self.max_x - 27  # Leave 27 columns for player list
        ...
        with self.log_lock:
            ...
            if total_lines <= log_height:
                ...
                # When scroll_offset = 0, show newest lines (last log_height lines)
                # When scroll_offset > 0, show older lines by moving start_idx back
                start_idx = max(0, total_lines - log_height - self.scroll_offset)
                end_idx = min(total_lines, start_idx + log_height)
                display_lines = all_lines[start_idx:end_idx]

                # Ensure we always show the newest lines when at scroll_offset = 0
                if self.scroll_offset == 0 and len(display_lines) < log_height:
                    # This shouldn't happen, but just in case, ensure we show newest lines
                    start_idx = max(0, total_lines - log_height)
                    end_idx = total_lines
                    display_lines = all_lines[start_idx:end_idx]

        # Check if log content changed or if we need to force redraw
        current_content = "".join(display_lines)
        if current_content == self._last_log_content and not self._needs_redraw and not self.is_dirty('log'):
            return  # No change, skip drawing
        ...
        for y in range(log_start_y, log_end_y):
            sys.stdout.write(TerminalControl.move_cursor(y, 1) + " " * log_width)
        ...
        for line in display_lines:
            ...
            # Wrap long lines
            wrapped_lines = self.wrap_line(clean_line, log_width)
            ...
            for wrapped_line in wrapped_lines:
                ...
                display_line = wrapped_line[:log_width] if len(wrapped_line) > log_width else wrapped_line

                # Draw with proper spacing
                sys.stdout.write(TerminalControl.move_cursor(current_y, 1) + color + display_line + TerminalColors.RESET)
                current_y += 1
                lines_drawn += 1
    ...
    def draw_player_list(self):
        ...
        player_list_x = self.max_x - 25  # Start 25 columns from right edge
        player_list_width = 25  # 25 columns wide
        player_list_start_y = 1  # Start at the top (row 1)
        player_list_end_y = self.max_y  # End at the bottom (row self.max_y)
        ...
        for y in range(player_list_start_y + 3, player_list_end_y):
            sys.stdout.write(TerminalControl.move_cursor(y, player_list_x) + " " * player_list_width)
        ...
        if players:
            for i, player in enumerate(players):
                if player_list_start_y + 3 + i < player_list_end_y:  # Make sure we don't overflow
                    ...
                    display_name = player[:player_list_width - 4] if len(player) > player_list_width - 4 else player
                    player_text = f" 🟢 {display_name} "
                    if len(player) > player_list_width - 4:
                        player_text = player_text[:-3] + "… "

                    sys.stdout.write(TerminalControl.move_cursor(player_list_start_y + 3 + i, player_list_x) + TerminalColors.BRIGHT_CYAN + player_text + TerminalColors.RESET)
        else:
    ...
    def scroll_log_up(self):
        ...
        with self.log_lock:
            ...
            log_height = self.max_y - 10  # Account for feedback frame and help lines (no borders)
    ...
    def process_command(self):
        ...
        if cmd in ['start', 's']:
            ...
            # Completely non-blocking restart
    ...
    def cleanup(self):
        ...
        self.wait_for_background_tasks(timeout=1.0)  # Reduced timeout
        ...
        try:
            ...
            while self.running:
                ...
                try:
                    ...
                    # Clear any error messages that might be stuck
    ...
    def clear_error_messages(self):
        """Clear any error messages that might be stuck in the UI"""
        # Clear any lines that might have error messages below the log area
        log_end_y = self.max_y - 6  # Account for feedback frame and help lines (no borders)
        for y in range(log_end_y, self.max_y - 3):  # Clear area below log but above help
            sys.stdout.write(TerminalControl.move_cursor(y, 1) + TerminalControl.clear_line())
        sys.stdout.flush()
    ...
    def start_background_task(self, task_func, *args, **kwargs):
        ...
        # Create a wrapper function with proper exception handling
        def safe_task_wrapper():
            try:
                return task_func(*args, **kwargs)
            except Exception as e:
                # Log the error properly instead of printing to stdout
                self.log_error(f"Background task error in {task_func.__name__}: {e}")
                return None

        # Create and start the thread
        thread = threading.Thread(target=safe_task_wrapper, daemon=True)
        thread.start()
        self._background_tasks.append(thread)
        return thread

    def wait_for_background_tasks(self, timeout=1.0):
    ...
    def cleanup_memory(self):
        ...
        with self.log_lock:
            if len(self.log_lines) > self.PERFORMANCE['log_buffer_max_size'] * 0.9:  # Keep under 90% of max
                # Remove oldest lines, keeping recent ones
                target_size = int(self.PERFORMANCE['log_buffer_max_size'] * 0.9)
                excess = len(self.log_lines) - target_size
                for _ in range(excess):
                    self.log_lines.popleft()
                # Mark log area as dirty after trimming
                self.mark_dirty('log')
